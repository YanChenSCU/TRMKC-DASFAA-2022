function [K, H, w, objHistory, objHistory2] = OKL(Ks, nCluster, w_type, is_adaptive_delta)
global t0 delta2_0;
...
while ~converges_1
    ...
    while ~converges_2
        ...
        switch w_type
            case 'EDWS_SCMK'
                K = update_K_psd(Ks, w, HHT, lambda_2);
            case 'WCIM_JMKSC'
                K = update_K_psd(Ks, w, HHT, lambda_2);
            case 'KAWS_SPMKC'
                K = update_K_psd(Ks, w, HHT, lambda_2);
    ...
    switch w_type
        ...
        case 'WCIM_JMKSC'
            ...
            delta2 = 1./(2 * nSmp * nSmp * e); %Eq. 26
            
            w = zeros(nKernel, 1);
            for iKernel = 1:nKernel
                E = K -  Ks(:, :, iKernel);
                E2 = E.^2/( 2 * delta2(iKernel));
                E2 = exp(-E2);
                w(iKernel) = 1 - sqrt(1 - sum(sum(E2))/(nSmp * nSmp)); %Eq. 27
            end
        case 'KAWS_SPMKC'
            e = zeros(nKernel, 1);
            for iKernel = 1:nKernel
...
function f2 = compute_f2_wls(Ks, w, K)
[~,~,nKernel] = size(Ks);
e = zeros(nKernel, 1);
for iKernel = 1:nKernel
    E = K -  Ks(:, :, iKernel);
